...
var (
	...
	once        sync.Once
	currentCore zapcore.Core        // 保存当前的 core，用于后续添加 OTEL
	writeSyncer zapcore.WriteSyncer // 保存 writeSyncer
	encoder     zapcore.Encoder     // 保存 encoder
	logLevel    zapcore.Level       // 保存日志级别
)
...
type Config struct {
	...
	SplitLevel   bool   // 是否按级别分割: debug/info -> app.log, warn/error -> app.error.log, panic/fatal -> app.panic.log
}
...
func initLogger(cfg *Config) error {
	...
	case "file":
		if cfg.SplitLevel {
			currentCore = createSplitLevelCore(cfg, encoder, level)
			// hertz logger 需要 writeSyncer，使用主日志文件
			writeSyncer = getFileWriter(cfg, cfg.Filename)
		} else {
			writeSyncer = getFileWriter(cfg, cfg.Filename)
			currentCore = zapcore.NewCore(encoder, writeSyncer, level)
		}
	case "both":
		if cfg.SplitLevel {
			fileCore := createSplitLevelCore(cfg, encoder, level)
			stdoutCore := zapcore.NewCore(encoder, zapcore.AddSync(os.Stdout), level)
			currentCore = zapcore.NewTee(stdoutCore, fileCore)
			// hertz logger 需要 writeSyncer
			writeSyncer = zapcore.NewMultiWriteSyncer(
				zapcore.AddSync(os.Stdout),
				getFileWriter(cfg, cfg.Filename),
			)
		} else {
			writeSyncer = zapcore.NewMultiWriteSyncer(
				zapcore.AddSync(os.Stdout),
...
// createSplitLevelCore 创建按级别分割的 core
// debug/info -> app.log, warn/error -> app.error.log, dpanic/panic/fatal -> app.panic.log
func createSplitLevelCore(cfg *Config, enc zapcore.Encoder, minLevel zapcore.Level) zapcore.Core {
	// 构建日志文件名: app.log -> app.error.log, app.panic.log
	ext := filepath.Ext(cfg.Filename)
	baseName := cfg.Filename[:len(cfg.Filename)-len(ext)]
	errorFilename := baseName + ".error" + ext
	panicFilename := baseName + ".panic" + ext

	// 创建三个写入器
	infoWriter := getFileWriter(cfg, cfg.Filename)
	errorWriter := getFileWriter(cfg, errorFilename)
	panicWriter := getFileWriter(cfg, panicFilename)

	// info core: 只处理 debug 和 info 级别
	...
	// error core: 只处理 warn 和 error 级别
	errorCore := zapcore.NewCore(
		enc,
		errorWriter,
		zap.LevelEnablerFunc(func(lvl zapcore.Level) bool {
			return lvl >= zapcore.WarnLevel && lvl < zapcore.DPanicLevel
		}),
	)

	// panic core: 处理 dpanic/panic/fatal 级别
	...
	return zapcore.NewTee(infoCore, errorCore, panicCore)
}
...
// EnableOTEL 启用 OpenTelemetry 日志导出
// 使用 otelzap bridge 将 zap 日志同时输出到本地和 OTEL
// 必须在 OTEL LoggerProvider 初始化后调用
func EnableOTEL(serviceName string) error {
	if currentCore == nil {
		return nil // logger 未初始化，跳过
	}

	// 获取全局 OTEL LoggerProvider
	provider := global.GetLoggerProvider()

	// 创建 otelzap core，使用全局 provider
	otelCore := otelzap.NewCore(serviceName,
		otelzap.WithLoggerProvider(provider),
	)

	// 使用 NewTee 组合原有 core 和 otelzap core
	// 日志会同时输出到本地（stdout/file）和 OTEL
